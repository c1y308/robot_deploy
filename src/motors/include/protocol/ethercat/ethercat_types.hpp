...
inline bool is_internal_limit_active_bit(uint16_t sw)
{
    return (sw & BIT_INTERNAL_LIMIT_ACTIVE) != 0;
}
...
static_assert(sizeof(TxPDO) == TX_PDO_SIZE);
static_assert(sizeof(RxPDO) == RX_PDO_SIZE);
