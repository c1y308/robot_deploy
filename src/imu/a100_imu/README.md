...
├── include/                # 头文件目录
│   ├── data_types.h        # 数据类型定义
│   ├── serial_port.h       # 串口驱动头文件
│   └── imu_parser.h        # 数据解析头文件
└── src/                    # 源文件目录
    ├── main.c              # 主程序
    ├── serial_port.c       # 串口驱动实现
    └── imu_parser.c        # 数据解析实现
```
...
```bash
# 基本用法
./imu_reader -d /dev/ttyS1 -b 921600

# 显示帮助
./imu_reader -h

# 只打印IMU数据
./imu_reader -d /dev/ttyS1 -b 921600 -p

# 打印IMU和AHRS数据
./imu_reader -d /dev/ttyS1 -b 921600 -p -a

# 打印所有数据并显示统计信息
./imu_reader -d /dev/ttyS1 -b 921600 -p -a -g -s
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `-d, --device` | 串口设备路径 | `/dev/ttyS1` |
| `-b, --baud` | 波特率 | `921600` |
| `-p, --print-imu` | 打印IMU数据 | 关闭 |
| `-a, --print-ahrs` | 打印AHRS数据 | 关闭 |
| `-g, --print-insgps` | 打印INSGPS数据 | 关闭 |
| `-s, --stats` | 打印统计信息 | 关闭 |
| `-h, --help` | 显示帮助 | - |
