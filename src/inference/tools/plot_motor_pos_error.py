#!/usr/bin/env python3
"""Plot TAR_ERR_DEG curves from motor_pos_error CSV logs."""
...
def default_log_dir() -> Path:
    return Path(__file__).resolve().parents[1] / "log" / "motor_pos_error"


def find_latest_csv(log_dir: Path) -> Path:
    candidates = sorted(
        log_dir.glob("motor_pos_error_*.csv"),
        key=lambda path: path.stat().st_mtime,
    )
    if not candidates:
        raise FileNotFoundError(f"no motor_pos_error_*.csv files under {log_dir}")
    return candidates[-1]
...
def load_rows(csv_path: Path) -> tuple[list[dict[str, str]], list[int]]:
    ...
    motors.sort()
    if not motors:
        raise ValueError(f"{csv_path} has no tar_err_deg_* columns")
    return rows, motors


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot TAR_ERR_DEG curves from src/inference/log/motor_pos_error CSV logs."
    )
    parser.add_argument(
        "csv",
        nargs="?",
        type=Path,
        help="CSV file to plot. Defaults to the latest motor_pos_error_*.csv.",
    )
    parser.add_argument(
        "-m",
        "--motors",
        help="Comma-separated motor IDs to plot, for example 0,1,5. Defaults to all.",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--abs",
        action="store_true",
        help="Plot absolute error magnitude instead of signed TAR_ERR_DEG.",
    )
    parser.add_argument(
        "--show",
        action="store_true",
...
def build_plot_data(
    rows: list[dict[str, str]], selected_motors: list[int], use_abs: bool
    ...
    for motor_id in selected_motors:
        column = f"tar_err_deg_{motor_id}"
...
def plot_with_matplotlib(
    ...
    title_suffix = "abs TAR_ERR_DEG" if use_abs else "TAR_ERR_DEG"
    ax.set_title(f"{csv_path.name} - {title_suffix}")
...
def plot_with_svg(
    ...
    lines: list[str] = [
        ...
        f"<text x=\"{left}\" y=\"28\" font-size=\"18\">{escape(csv_path.name)} - {title_suffix}</text>",
    ]
    ...
    lines.extend(
        [
            ...
            f'<text x="20" y="{top + plot_height / 2:.2f}" transform="rotate(-90 20 {top + plot_height / 2:.2f})" text-anchor="middle">error (deg)</text>',
        ]
    )
...
def main() -> int:
    ...
    x_values, x_label, series = build_plot_data(rows, selected_motors, args.abs)
    ...
    if args.output:
        ...
        output_path = csv_path.with_suffix(".png" if has_matplotlib else ".svg").resolve()
    ...
    if has_matplotlib:
        ...
        if args.show:
            print("matplotlib is not installed; --show is ignored for SVG output", file=sys.stderr)
        plot_with_svg(csv_path, output_path, x_values, x_label, series, args.abs)
